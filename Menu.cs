  ...
  public partial class Menu : Form
  {
      ...
      while (checkBoxActive.Checked)
      {
        ...
        for (int i = 0; i < 4; i++)
        {
          ...
          if ((matched / AGENTPIXELS) >= REQUIRED_ACCURACY)
          {
            ...
            //checkBoxActive.Invoke((Action)(() => UncheckActive())); // Stop the search

            // Selection repeated for 3 seconds to make sure an agent is selected
    ...
    private void CheckBoxActive_CheckedChanged(object sender, EventArgs e)
    {
      if (checkBoxActive.Checked)
      {
        ...
        // One thread per row (6 rows)
        for (int i = 0; i < 6; i++)
        {
          int threadIndex = i;
          Thread searchThread = new Thread(() => SearchAgent(threadIndex))
          {
            IsBackground = true
          };
          searchThread.Start();
    ...
    private void SelectCloveButton_Click(object sender, EventArgs e)
    {
      if (!checkBoxRandomAgent.Checked)
      {
        agentName = "Clove";
